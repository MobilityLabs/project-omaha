...
describe Comment do
  before do
    @user = User.create!
    @comment = Comment.create!(body: 'Root comment', commentator: @user)
  end
  ...
  describe 'after having a child added' do
    before do
      @child = Comment.create!(body: 'Child comment', commentator: @user)
  ...
  describe 'finders' do
    describe '#find_comments_by_user' do
      before :each do
        ...
        @user_comment = Comment.create!(body: 'Child comment', commentator: @user)
        @non_user_comment = Comment.create!(body: 'Child comment',
                                            commentator: @other_user)
        @comments = Comment.find_comments_by_commentator(@user)
      end
